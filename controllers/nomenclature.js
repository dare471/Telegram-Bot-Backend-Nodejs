...
exports.getShippingByNumber = async (msg) => {
  ...
  const find = msg.text;
  await axios
    .get(`${config.ONE_C_URL}getShippingByNumber`, {
      headers: {
          ...
          bot.sendMessage(id, selectdocument, {
            reply_markup: {
              ...
              inline_keyboard: [
                [
                  {
                    text: resp[i].Представление,
                    callback_data: "predstavlenie",
                  },
                ],
              ],
            },
...
exports.getProductByName = async (msg) => {
  ...
  if (name.length < 4) {
    ...
    await axios
      .get(`${config.ONE_C_URL}getProductByName`, {
        headers: {
          "Content-Type": "application/json",
        },
        auth: {
      ...
      .then((res) => {
        ...
        if (Array.isArray(products) && products.length > 0) {
          ...
          for (let i = 0; i < products.length; i++) {
            btns.push([
              {
                text: products[i].Представление,
                callback_data: products[i].GUID,
              },
            ]);
          }
...
exports.getCategory = async (msg) => {
  ...
  await axios
    .get(`${config.ONE_C_URL}getCategory`, {
      headers: {
        "Content-Type": "application/json",
      },
      auth: {
    ...
    .then((res) => {
      ...
      if (Array.isArray(category) && category.length > 0) {
        ...
        for (let i = 0; i < category.length; i++) {
          btns.push([
            {
              text: category[i].Представление,
              callback_data: category[i].GUID,
            },
          ]);
        }
        bot
          .sendMessage(id, chosecategory, {
            reply_markup: {
              resize_keyboard: true,
              inline_keyboard: btns,
            },
...
exports.getProductByCategory1 = (msg) => {
  ...
  if (product.length > 10) {
    for (let i = 0; i < 10; i++) {
      btns.push([
        { text: product[i].Представление, callback_data: product[i].GUID },
      ]);
    }
    product.splice(0, 10);
    myTasks.setAllproducts(id, product);
    bot
      .sendMessage(id, choseproduct, {
        reply_markup: {
          resize_keyboard: true,
          inline_keyboard: btns,
        },
...
exports.getProductByCategory = async (id, guid) => {
  await axios
    .get(`${config.ONE_C_URL}getProductByCategory`, {
      headers: {
        "Content-Type": "application/json",
      },
      auth: {
      ...
      if (Array.isArray(product) && product.length > 0) {
        if (product.length > 20) {
          ...
          for (let i = 0; i < 20; i++) {
            btns.push([
              {
                text: product[i].Представление,
                callback_data: product[i].GUID,
              },
            ]);
          }
          product.splice(0, 20);
...
exports.getProductByGuid = async (id, guid) => {
  ...
  await axios
    .get(`${config.ONE_C_URL}getProductByGuid`, {
      headers: {
        "Content-Type": "application/json",
      },
      auth: {
...
exports.getStockByProduct = async (msg) => {
  ...
  await axios
    .get(`${config.ONE_C_URL}getStockByProduct`, {
      responseType: "stream",
      auth: {
        username: config.ONE_C_AUTH_LOGIN,
        password: config.ONE_C_AUTH_PASSWORD,
    ...
    .then((res) => {
      const path = Path.resolve(
        __dirname,
        "../dist/pdf",
        `Все склады остатки товара ${guid}-${now}-a.${res.headers.file_type}`
      );
      ...
      bot
        .sendDocument(
          id,
          encodeURI(
            `${config.BASE_URL}pdf/Все склады остатки товара ${guid}-${now}-a.${res.headers.file_type}`
          )
        )
        .then(() => {
          fs.unlink(
            `dist/pdf/Все склады остатки товара ${guid}-${now}-a.${res.headers.file_type}`,
            (err) => {
...
exports.getStocks = async (msg) => {
  const { id } = msg.from;
  await axios
    .get(`${config.ONE_C_URL}getStocks`, {
      headers: {
        "Content-Type": "application/json",
      },
      auth: {
    ...
    .then((res) => {
      ...
      if (Array.isArray(stocks) && stocks.length > 0) {
        ...
        for (let i = 0; i < stocks.length; i++) {
          btns.push([
            { text: stocks[i].Представление, callback_data: stocks[i].GUID },
          ]);
        }
        bot
          .sendMessage(id, chosewarehouse, {
            reply_markup: {
              resize_keyboard: true,
              inline_keyboard: btns,
            },
...
exports.getStockByProductAndStock = async (id, guid) => {
  ...
  await axios
    .get(`${config.ONE_C_URL}getStockByProductAndStock`, {
      responseType: "stream",
      auth: {
        username: config.ONE_C_AUTH_LOGIN,
        password: config.ONE_C_AUTH_PASSWORD,
    ...
    .then((res) => {
      const path = Path.resolve(
        __dirname,
        "../dist/pdf",
        `Остатки товара ${guid_product}${guid}-${now}-a.${res.headers.file_type}`
      );
      const writer = fs.createWriteStream(path);
      res.data.pipe(writer);
      bot
        .sendDocument(
          id,
          encodeURI(
            `${config.BASE_URL}pdf/Остатки товара ${guid_product}${guid}-${now}-a.${res.headers.file_type}`
          )
        )
        .then(() => {
          fs.unlink(
            `dist/pdf/Остатки товара ${guid_product}${guid}-${now}-a.${res.headers.file_type}`,
            (err) => {
              if (err && err.code == "ENOENT") {
                // file doens't exist
                console.info("File doesn't exist, won't remove it.");
...
exports.getTypeCertByProduc = async (msg) => {
  ...
  let guid = myTasks.getUserType()[id];
  await axios
    .get(`${config.ONE_C_URL}getTypeCertByProduct`, {
      headers: {
        "Content-Type": "application/json",
      },
      auth: {
    ...
    .then((res) => {
      ...
      if (Array.isArray(cert) && cert.length > 0) {
        ...
        for (let i = 0; i < cert.length; i++) {
          btns.push([
            {
              text: cert[i].Представление,
              callback_data: cert[i].Представление,
            },
          ]);
        }
        bot
          .sendMessage(id, entertypecert, {
            reply_markup: {
              resize_keyboard: true,
              inline_keyboard: btns,
            },
...
exports.getCertByProduct = async (id, type) => {
  ...
  await axios
    .post(`${config.ONE_C_URL}getCertByProduct`, data, {
      headers: {
        "Content-Type": "application/json",
      },
      auth: {
    ...
    .then((res) => {
      ...
      if (Array.isArray(cert) && cert.length > 0) {
        ...
        for (let i = 0; i < cert.length; i++) {
          btns.push([
            { text: cert[i].Представление, callback_data: cert[i].GUID },
          ]);
        }
        bot
          .sendMessage(id, entercert, {
            reply_markup: {
              resize_keyboard: true,
              inline_keyboard: btns,
            },
...
exports.getFilesCertByGUID = async (id, guid) => {
  await axios
    .get(`${config.ONE_C_URL}getFilesCertByGUID`, {
      responseType: "stream",
      auth: {
        username: config.ONE_C_AUTH_LOGIN,
        password: config.ONE_C_AUTH_PASSWORD,
    ...
    .then((res) => {
      const path = Path.resolve(
        __dirname,
        "../dist/pdf",
        `Сертификат ${guid}-${now}-a.${res.headers.file_type}`
      );
      const writer = fs.createWriteStream(path);
      res.data.pipe(writer);
      bot
        .sendDocument(
          id,
          encodeURI(
            `${config.BASE_URL}pdf/Сертификат ${guid}-${now}-a.${res.headers.file_type}`
          )
        )
        .then(() => {
          fs.unlink(
            `dist/pdf/Сертификат ${guid}-${now}-a.${res.headers.file_type}`,
            (err) => {
              if (err && err.code == "ENOENT") {
                // file doens't exist
                console.info("File doesn't exist, won't remove it.");
...
exports.getStockByStock = async (id, guid) => {
  await axios
    .get(`${config.ONE_C_URL}getStockByStock`, {
      responseType: "stream",
      auth: {
        username: config.ONE_C_AUTH_LOGIN,
        password: config.ONE_C_AUTH_PASSWORD,
    ...
    .then((res) => {
        ...
        const path = Path.resolve(
            __dirname,
            '../dist/pdf',
            `Склад-${guid}-${now}.${res.headers.file_type}`,
        )
        const writer = fs.createWriteStream(path)
        res.data.pipe(writer)
        console.log(res.data)
        bot.sendDocument(id, encodeURI(`${config.BASE_URL}pdf/Склад-${guid}-${now}.${res.headers.file_type}`))
        .then(() => {
          fs.unlink(`dist/pdf/Склад-${guid}-${now}.${res.headers.file_type}`, (err) => {
              if (err && err.code == 'ENOENT') {
                  // file doens't exis
                  console.info("File doesn't exist, won't remove it.")
              } else if (err) {
